...
typedef struct {
    ...
    byte keySched[ROUNDS+1][4][4];
    byte keySched2[ROUNDS+1][4][4];
} context;
...
static int
do_setkey (context *ctx, const byte key[16])
{
    byte k[MAXKC][4];
...
static void
prepare_decryption(context *ctx)
{
    int r;
    byte *w;
...
static void
do_encrypt (const context *ctx, byte *b, const byte *a)
{
    int r;
    byte temp[4][4];
...
static void
do_decrypt (context *ctx, byte *b, const byte *a)
{
...
void
encryptdata (byte *data, u32 len, const byte key[16])
{
    unsigned int i, j;
    char iv[16];
    context ctx;
    ...
    for (i = 0 ; i < len/16 ; i++) {
	for (j = 0 ; j < 16 ; j += 4)
	    *(u32*)&data[j] = *(u32*)&data[j] ^ *(u32*)&iv[j];
	do_encrypt(&ctx, data, data);
	memcpy(iv, data, 16);
	data += 16;
    }
}
...
void
decryptdata (byte *data, u32 len, const byte key[16])
{
    unsigned int i, j;
    char lastiv[16], iv[16];
    context ctx;
    ...
    for (i = 0 ; i < len/16 ; i++) {
	memcpy(lastiv, data, 16);
	do_decrypt(&ctx, data, data);
	for (j = 0 ; j < 16 ; j += 4)
	    *(u32*)&data[j] = *(u32*)&data[j] ^ *(u32*)&iv[j];
	memcpy(iv, lastiv, 16);
	data += 16;
