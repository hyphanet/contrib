// fcpput.c - simple command line client that uses FCP
...
int main(int argc, char* argv[])
{
  ...
  if (metaFile != NULL) {
	 ...
	 FILE *fp_meta = meta_stdin ? stdin	: fopen(metaFile, "r");
	 char metaLine[256];
  ...
  if (keyFile == NULL) {
	 ...
	 if (fileSize < MAX_KSK_LEN || strstr(keyUri, "CHK@")) {
		...
		do _fcpLog(FCP_LOG_VERBOSE,	"Key is small or chk, inserting directly, attempt %d/%d", i, maxAttempts);
...
static void parse_args(int argc, char *argv[])
{
  ...
  if (argc == 1) usage("missing key URI");
...
static void *usage(char *s)
{
  ...
  printf("  -m file        Get key's metadata from file, \"stdout\" means stdout\n");
  ...
  printf("Therefore, you can use this utility in shell commands (via backticks)\n");
