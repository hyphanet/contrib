                    ...
                    try {
                        ...
                        while (rs.size() != b.length) {
                            if (rs.isEmpty()) {
                                ...
                                min = (int) ((Range) rs.iterator().next()).getMax()
                                      + rand.nextInt(b.length - (int) rs.size());
                            }

                            max = rand.nextInt(b.length - min) + min;

                            // See where this range overlaps.
                            RangeSet available = rs.complement().intersect(new RangeSet(new Range(min, max)));

                            if (!available.isEmpty()) {
                                Range r = (Range) available.iterator().next();
