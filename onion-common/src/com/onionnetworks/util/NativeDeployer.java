...
public class NativeDeployer {
    ...
    static {
        final String OS = System.getProperty("os.name").startsWith("Windows ")
                          ? "win32" : System.getProperty("os.name").toLowerCase();
    ...
    public synchronized final static String getLibraryPath(ClassLoader cl, String libName) {
        ...
        try {
            ...
            System.out.println("It took " + (System.currentTimeMillis() - t) + " millis to extract " + libName);
    ...
    public synchronized final static String getLocalResourcePath(ClassLoader cl, String resourcePath)
            throws IOException {
     ...
     */
    private final static HashMap findLibraries(ClassLoader cl) throws IOException {
        ...
        for (Enumeration en = cl.getResources(NATIVE_PROPERTIES_PATH); en.hasMoreElements(); ) {
            ...
            for (StringTokenizer st = new StringTokenizer(p.getProperty("com.onionnetworks.native.keys"), ",");
                    st.hasMoreTokens(); ) {
                String key = st.nextToken().trim();

                // If it matches the os and arch then add it.
                if (p.getProperty("com.onionnetworks.native." + key + ".osarch").trim().equals(OS_ARCH)) {
                    libMap.put(p.getProperty("com.onionnetworks.native." + key + ".name").trim(),
